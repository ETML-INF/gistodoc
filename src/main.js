...
async function ghGQL(pat, query, variables = {}) {
  ...
  if (status !== 200) {
    let detail = resp;
    try {
      detail = JSON.parse(resp).message ?? resp;
    } catch {
      /* keep raw body */
    }
    throw new Error(`GraphQL HTTP ${status} — ${detail}`);
  }
...
function parseRepoUrl(url) {
  ...
  if (!m) throw new Error("URL non reconnue. Format : https://github.com/OWNER/REPO");
  return { owner: m[1], repo: m[2] };
}

// ── Fetch repository issues labeled "US" via GraphQL ─────────────────────────

async function fetchRepoIssues(pat, owner, repo) {
  const allNodes = [];
  let cursor = null;
  ...
  while (hasNextPage) {
    const data = await ghGQL(
      ...
      `query($owner: String!, $repo: String!, $cursor: String) {
        repository(owner: $owner, name: $repo) {
          issues(first: 100, after: $cursor, labels: ["US"], states: [OPEN, CLOSED]) {
    ...
    if (!data.repository) throw new Error("Dépôt introuvable. Vérifiez l'URL et le PAT.");
    allNodes.push(...data.repository.issues.nodes);
...
async function fetchProjectData(url, pat, onProgress) {
  ...
  onProgress('Récupération des User Stories (label "US")…');
  const rawIssues = await fetchRepoIssues(pat, owner, repo);
  onProgress(`${owner}/${repo} — ${rawIssues.length} User Stor${rawIssues.length !== 1 ? "ies" : "y"}`);

  const issues = rawIssues.map((c) => ({ number: c.number, title: c.title, body: c.body ?? "", images: new Map() }));
  ...
  if (totalImages > 0)
    onProgress(`${totalImages} image${totalImages !== 1 ? "s" : ""} téléchargée${totalImages !== 1 ? "s" : ""}.`);

  return { projectTitle: `${owner}/${repo}`, url, fetchedAt: new Date().toISOString(), issues };
}
...
function formatMarkdown({ projectTitle, url, fetchedAt, issues }) {
  ...
  const lines = [
    `# ${projectTitle}`,
    "",
    `*Exporté le ${date} — ${issues.length} User Stor${issues.length !== 1 ? "ies" : "y"}*`,
    `*Source : ${url}*`,
    "",
    "---",
    ""
...
async function buildDocxDocument({ projectTitle, url, fetchedAt, issues }) {
  ...
  const children = [
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: `Exporté le ${date} — ${issues.length} issue${issues.length !== 1 ? "s" : ""}`,
          italics: true
        })
      ]
    }),
  ...
  ipcMain.handle("export", async (event, { url, pat, format }) => {
    try {
      ...
      if (data.issues.length === 0) {
        return { success: false, error: "Aucune User Story avec l'étiquette \"US\" n'a été trouvée dans ce repo." };
      }
